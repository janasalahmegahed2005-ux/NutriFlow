...
export class GoalsComponent {
  ...
  get calorieTarget(): number {
    return this.user?.calorieTarget || 2000;
  }

  get proteinTarget(): number {
    return this.user?.proteinTarget || 120;
  }

  get carbsTarget(): number {
    return this.user?.carbsTarget || 250;
  }

  get fatTarget(): number {
    return this.user?.fatTarget || 65;
  }

  get goal(): string {
    return this.user?.goal || 'No goal set yet.';
  }
  ...
  get bmi(): number {
    ...
    return Math.round(
      (this.weight / (heightM * heightM)) * 10
    ) / 10;
  }
